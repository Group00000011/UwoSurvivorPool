...
public class GameSettings extends JPanel implements ActionListener {
	...
	// Stores the number of Contestants
	private int numContestants;

	// Stores the amount of money per player in the field
	private int money;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("save")) {
			...
			if (Integer.parseInt(contField.getText()) >= 6 && Integer.parseInt(contField.getText()) <= 15){
				numContestants = Integer.parseInt(contField.getText());
			}
			else{ // otherwise reset the field
				contField.setText("");
			}
			//the amount wagered per person
			money = Integer.parseInt(wagerField.getText());
		}		
	}
	
	/**
	 * Getter for retrieving the number of contestants
	...
	/**
	 * Getter for retrieving the amount of money wagered per person
	 * @return The amount of money wagered per person
	 */
	public int getWager(){
		return money;
	}
