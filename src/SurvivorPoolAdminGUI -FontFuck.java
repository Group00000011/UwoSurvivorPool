...
public class SurvivorPoolAdminGUI extends JFrame implements ActionListener {
	...
	private ImageIcon title, quitImg;
	private ImageIcon goldBackground, jungleBackground;
	private ImageIcon newGame, playersJungleImg, playersGoldImg, contestantImg, stadingsImg, bonusQImg, themeSelectImg;
	private ImageIcon playerJBg, playerGBg, contestantJBg, contestantGBg, blankGFrame, blankJFrame;
	
	// Buttons
	private JButton quitBtn, createNewGameBtn, playersBtn, contestantsBtn, standingsBtn, bonusQBtn, themeSelectBtn;		

	// JLabels
	private JLabel themeMaker, playerBg, contestantBg, titleBanner, contestantPicFrame;
	
	// JPanels
	private JPanel mainPanel, quitPanel, titlePanel, mainButtonsPanel, pPanel, leftHalf_p, playerPanel, cPanel, contestantPanel;
 ...
 */
	private JMenuBar menuBar() {
		...
		startGameItem.setActionCommand("start");
	...
	private JComponent mainScreen() {
	    ...
	    themeMaker = new JLabel(goldBackground);
...
//	    getContentPane().add(goldenRuinsTheme());
	    goldenRuinsTheme();
	...
	protected JComponent playersPanel() {
		...
		// Create a seperator
		leftHalf_p.setBorder(BorderFactory.createEmptyBorder(	GAP/2, //top
												                0,     //left
												                GAP/2, //bottom
												                0));   //right
		...
		pPanel.add(textFields_p.createRecordDisplay());
	...
	protected JComponent contestantPanel() {
	    ...
	    rightHalf_c.add(textFields_c.createRecordDisplay());
	    rightHalf_c.setPreferredSize(new Dimension(492,580));
	    
	    cPanel.setOpaque(false);
	    cPanel.add(leftHalf_c);
	    cPanel.add(Box.createRigidArea(new Dimension(GAP,0)));
	    cPanel.add(rightHalf_c);
...
//		JPanel goldenRuinsPanel = new JPanel();
...
//		return junglePanel;		
	...
	public void actionPerformed(ActionEvent e) {		
		...
		/**  Main Menu Button Handler **/
			...
			if(playersPanel().isVisible()) {
				playerBg.setVisible(false);
				pPanel.setVisible(false);
//				getContentPane().remove(pPanel);
				leftHalf_p.setVisible(false);
				playerPanel.setVisible(false);
			}
//			if(contestantPanel.isVisible()) {
//				contestantBg.setVisible(false);
...
//			}
		...
		/**  Players List Button Handler **/
			...
			playerBg.setIcon(playerBg.getIcon());
			textFields_p.getGameFont();
			textFields_p.getGameFontColor();
			
			themeMaker.setVisible(false);
			mainButtonsPanel.setVisible(false);
			titleBanner.setVisible(false);
						
			getContentPane().add(playersPanel());
			getContentPane().add(textFields_p.setGameFont(getFont(), Color.WHITE));
//			validate();
		}
		/**  Contestants List Button Handler **/
		else if(e.getActionCommand().equals("contestants")) {
			contestantBg.setIcon(contestantBg.getIcon());
			textFields_c.getGameFont();
			textFields_c.getGameFontColor();
...
//			if(contestantBg.getIcon().equals(goldBackground)) {
////				getContentPane().add(textFields_c.setGameFont(gFont, Color.WHITE));
//				contestantBg.setIcon(contestantGBg);
//			}
//			else  {
////				getContentPane().add(textFields_c.setGameFont(jFont, Color.YELLOW));
//				contestantBg.setIcon(contestantJBg);
//			}
